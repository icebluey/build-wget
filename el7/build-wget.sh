...
_build_wget() {
    ...
    rm -f wget-*.tar*
    cd wget-*
    LDFLAGS='' ; LDFLAGS="${_ORIG_LDFLAGS}"' -Wl,-rpath,/usr/lib64/wget/private' ; export LDFLAGS
    ./configure \
    --build=x86_64-linux-gnu --host=x86_64-linux-gnu \
    --with-ssl=openssl --with-openssl=yes --with-cares --enable-largefile \
    --prefix=/usr --libdir=/usr/lib64 --includedir=/usr/include --sysconfdir=/etc
    sed '/ = /s| -Wl,-rpath -Wl,/usr/lib64||g' -i Makefile
    sed '/ = /s| -R/usr/lib64||g' -i Makefile
    sed '/ = /s| -Wl,-rpath -Wl,/usr/lib64||g' -i src/Makefile
    sed '/ = /s| -R/usr/lib64||g' -i src/Makefile
    make all
    rm -fr /tmp/wget
    ...
    echo
    install -m 0755 -d usr/lib64/wget
    cp -afr /usr/lib64/wget/private usr/lib64/wget/
    echo
    sleep 2
    tar -Jcvf /tmp/wget-"${_wget_ver}"-1.el7.x86_64.tar.xz *
    echo
    sleep 2
    cd /tmp
    openssl dgst -r -sha256 wget-"${_wget_ver}"-1.el7.x86_64.tar.xz | sed 's|\*| |g' > wget-"${_wget_ver}"-1.el7.x86_64.tar.xz.sha256
    rm -fr "${_tmp_dir}"
    rm -fr /tmp/wget
    /sbin/ldconfig
}
